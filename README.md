...
- A primary key is used as the only distinct value that keeps it unique. Emails, names, addresses, phone numbers can all change but the primary key should never change.

3. What is the name given to a table column that references the primary key on another table.

- A foriegn key allows a table to reference another table showing it's reltionship. For example the foriegn key on table two will point to the primary key on table one.

4. What do we need in order to have a _many to many_ relationship between two tables.

- You need a junction table to combine both tables
